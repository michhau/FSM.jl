function snow(ebm::EBM, cn::Constants, Sf, Rf, Ta, D, S, W, irow::Int = 1, icol::Int = 1)
    ...
    for col in 1:icol
        for row in 1:irow
            ...
            Roff = Rf[row, col] * ebm.dt

            if (ebm.Nsnow[row, col] > 0)   # Existing snowpack
                # Heat capacity
                for k = 1:ebm.Nsnow[row, col]
                    ebm.snow_csnow[k] = ebm.Sice[row, col, k] * cn.hcap_ice + ebm.Sliq[row, col, k] * cn.hcap_wat
                end

                # Heat conduction
                if (ebm.Nsnow[row, col] == 1)
                    ebm.snow_Gs[1] = 2.0 / (ebm.Ds[row, col, 1] / ebm.ksnow[row, col, 1] + ebm.Dzsoil[1] / ebm.ksoil[row, col, 1])
                    ebm.snow_dTs[1] = (ebm.Gsurf[row, col] + ebm.snow_Gs[1] * (ebm.Tsoil[row, col, 1] - ebm.Tsnow[row, col, 1])) * ebm.dt / (ebm.snow_csnow[1] + ebm.snow_Gs[1] * ebm.dt)
                else
                    for k = 1:(ebm.Nsnow[row, col] - 1)
                        ebm.snow_Gs[k] = 2.0 / (ebm.Ds[row, col, k] / ebm.ksnow[row, col, k] + ebm.Ds[row, col, k + 1] / ebm.ksnow[row, col, k + 1])
                    end
                    ebm.snow_a[1] = 0.0
                    ebm.snow_b[1] = ebm.snow_csnow[1] + ebm.snow_Gs[1] * ebm.dt
                    ebm.snow_c[1] = - ebm.snow_Gs[1] * ebm.dt
                    ebm.snow_rhs[1] = (ebm.Gsurf[row, col] - ebm.snow_Gs[1] * (ebm.Tsnow[row, col, 1] - ebm.Tsnow[row, col, 2])) * ebm.dt
                    for k = 2:(ebm.Nsnow[row, col] - 1)
                        ebm.snow_a[k] = ebm.snow_c[k - 1]
                        ebm.snow_b[k] = ebm.snow_csnow[k] + (ebm.snow_Gs[k - 1] + ebm.snow_Gs[k]) * ebm.dt
                        ebm.snow_c[k] = - ebm.snow_Gs[k] * ebm.dt
                        ebm.snow_rhs[k] = ebm.snow_Gs[k - 1] * (ebm.Tsnow[row, col, k - 1] - ebm.Tsnow[row, col, k]) * ebm.dt + ebm.snow_Gs[k] * (ebm.Tsnow[row, col, k + 1] - ebm.Tsnow[row, col, k]) * ebm.dt
                    end
                    k = ebm.Nsnow[row, col]

                    ebm.snow_Gs[k] = 2.0 / (ebm.Ds[row, col, k] / ebm.ksnow[row, col, k] + ebm.Dzsoil[1] / ebm.ksoil[row, col, 1])
                    ebm.snow_a[k] = ebm.snow_c[k - 1]
                    ebm.snow_b[k] = ebm.snow_csnow[k] + (ebm.snow_Gs[k - 1] + ebm.snow_Gs[k]) * ebm.dt
                    ebm.snow_c[k] = 0.0
                    ebm.snow_rhs[k] = ebm.snow_Gs[k - 1] * (ebm.Tsnow[row, col, k - 1] - ebm.Tsnow[row, col, k]) * ebm.dt + ebm.snow_Gs[k] * (ebm.Tsoil[row, col, 1] - ebm.Tsnow[row, col, k]) * ebm.dt
                    tridiag(ebm.Nsnow[row, col], ebm.snow_gamma, ebm.snow_a, ebm.snow_b, ebm.snow_c, ebm.snow_rhs, ebm.snow_dTs)
                end


                for k = 1:ebm.Nsnow[row, col]
                    ebm.Tsnow[row, col, k] = ebm.Tsnow[row, col, k] + ebm.snow_dTs[k]
                end
                ebm.Gsoil[row, col] = ebm.snow_Gs[ebm.Nsnow[row, col]] * (ebm.Tsnow[row, col, ebm.Nsnow[row, col]] - ebm.Tsoil[row, col, 1])
                ...
                for k = 1:ebm.Nsnow[row, col]
                    coldcont = ebm.snow_csnow[k] * (cn.Tm - ebm.Tsnow[row, col, k])
                    if (coldcont < 0.0)
                        # global dSice  ### HACK
                ...
                elseif ebm.hm[row, col] == 1  #  Bucket storage
                    for k = 1:ebm.Nsnow[row, col]
                        ...
                        if (coldcont > 0.0)            # Liquid can freeze
                            ...
                            ebm.Sliq[row, col, k] = ebm.Sliq[row, col, k] - dSice
                            ebm.Sice[row, col, k] = ebm.Sice[row, col, k] + dSice
                            ebm.Tsnow[row, col, k] = ebm.Tsnow[row, col, k] + cn.Lf * dSice / ebm.snow_csnow[k]
                        end
                    end
                end
            ...
            dSice = Sf[row, col] * ebm.dt - min(ebm.Esnow[row, col], 0.0) * ebm.dt
            ...
            if (ebm.Nsnow[row, col] == 0 && ebm.Sice[row, col, 1] > 0.0)
                ebm.Nsnow[row, col] = 1
                ebm.Tsnow[row, col, 1] = min(Ta[row, col], cn.Tm)
            end
            ...
            # global SWE
            # global snowdepth
                snowdepth[row, col] = snowdepth[row, col] + ebm.Ds[row, col, k]
                SWE[row, col] = SWE[row, col] + ebm.Sice[row, col, k] + ebm.Sliq[row, col, k]
            end
            ...
            for k = 1:ebm.Nsnow[row, col]
                ebm.snow_csnow[k] = ebm.Sice[row, col, k] * cn.hcap_ice + ebm.Sliq[row, col, k] * cn.hcap_wat
                ebm.snow_E[k] = ebm.snow_csnow[k] * (ebm.Tsnow[row, col, k] - cn.Tm)
            end
            ...
            if (SWE[row, col] > 0.0)       # Existing or new snowpack
                ...
                for kold = 1:Nold
                    while true
                        if (D[kold] < dnew)
                            ...
                            ebm.snow_U[knew] = ebm.snow_U[knew] + ebm.snow_E[kold]
                            dnew = dnew - D[kold]
                            break   ##### hack   exit
                        else
                            ...
                            ebm.snow_U[knew] = ebm.snow_U[knew] + wt * ebm.snow_E[kold]
                            D[kold] = (1 - wt) * D[kold]
                            ebm.snow_E[kold] = (1 - wt) * ebm.snow_E[kold]
                            S[kold] = (1 - wt) * S[kold]
                            W[kold] = (1 - wt) * W[kold]
                            knew = knew + 1
                ...
                for k = 1:ebm.Nsnow[row, col]
                    ebm.snow_csnow[k] = ebm.Sice[row, col, k] * cn.hcap_ice + ebm.Sliq[row, col, k] * cn.hcap_wat
                    if (ebm.snow_csnow[k] > eps(Float64))
                        ebm.Tsnow[row, col, k] = cn.Tm + ebm.snow_U[k] / ebm.snow_csnow[k]
                    end
                end
